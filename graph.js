function createlogin() {
  ...
  FormConn.style.textAlign = "center"; // Pour centrer horizontalement
  ...
  // Ajouter un élément pour afficher un message d'erreur
  let ErrorMessage = document.createElement("p");
  ErrorMessage.id = "ErrorMessage";
  ErrorMessage.style.color = "red"; // Message en rouge
  ErrorMessage.style.display = "none"; // Masqué par défaut
  ...
  FormConn.appendChild(ErrorMessage); // Ajout du message d'erreur
  body.appendChild(FormConn);

  // Ajout de la fonctionnalité pour la touche "Entrée"
  FormConn.addEventListener("keydown", function (event) {
    if (event.key === "Enter") {
  ...
  Submit.addEventListener("click", async function () {
    ...
    try {
      ...
      if (raiponce.ok) {
        ...
        ErrorMessage.style.display = "none"; // Masquer le message d'erreur si l'authentification est réussie
        DrawIntra(data);
      } else {
      ...
      // En cas d'erreur réseau, afficher un message générique
      ErrorMessage.innerText = "Erreur réseau, veuillez réessayer plus tard.";
      ErrorMessage.style.display = "block";
    }
  });
}
    ...
    .then((data) => {
      ...
      for (let i = 0; i < alltransaction.length; i++) {
        if (alltransaction[i].type.includes("skill_")) {
          alltransaction[i].type = alltransaction[i].type.replace(
            "skill_",
            "Skill "
          );
...
function createCircularRingSVG(
  percentage,
  type,
  radius = 115,
  strokeWidth = 10
